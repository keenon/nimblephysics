...
GTEST_TEST(dartc_simulation, set_get_gravity)
{
  ...
  const s_t* gravity = dart_world_get_gravity(world);
  EXPECT_s_t_EQ(gravity[0], 1);
  EXPECT_s_t_EQ(gravity[1], 2);
  EXPECT_s_t_EQ(gravity[2], 3);
  EXPECT_s_t_EQ(dart_world_get_gravity_x(world), 1);
  EXPECT_s_t_EQ(dart_world_get_gravity_y(world), 2);
  EXPECT_s_t_EQ(dart_world_get_gravity_z(world), 3);
  dart_world_destroy(world);
}
