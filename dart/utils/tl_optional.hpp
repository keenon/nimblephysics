...
#if (                                                                          \
    __cplusplus == 201103L || defined(TL_OPTIONAL_MSVC2015)                    \
...
template <
    ...
    typename = enable_if_t<
        !(is_pointer_to_non_const_member_func<Fn>::value
  ...
  : std::integral_constant<
        ...
        is_swappable<T, U>::value
            && ((decltype(detail::swap_adl_tests::uses_std<T, U>(0))::value&&
                ...
                || (!decltype(
                    detail::swap_adl_tests::uses_std<T, U>(0))::value&& detail::
...
// http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2017/p0826r0.html
  ...
  template <class F>
  constexpr decltype(
      optional_map_impl(std::declval<const optional&>(), std::declval<F&&>()))
  map(F&& f) const&
  {
    return optional_map_impl(*this, std::forward<F>(f));
  }
...
inline constexpr optional<T> make_optional(
...
template <
    ...
    class F,
    class Ret
    = decltype(detail::invoke(std::declval<F>(), *std::declval<Opt>())),
...
// http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2017/p0826r0.html
  ...
  template <class F>
  TL_OPTIONAL_11_CONSTEXPR decltype(
      detail::optional_map_impl(std::declval<optional&>(), std::declval<F&&>()))
  map(F&& f) &
  {
    return detail::optional_map_impl(*this, std::forward<F>(f));
  }
