...
std::shared_ptr<LossFn> getSSIDPosLoss()
{
  TrajectoryLossFn loss = [](const TrajectoryRollout* rollout) {
    ...
    Eigen::MatrixXs sensorPositions
        = rawPos.block(0, 1, rawPos.rows(), rawPos.cols() - 1);
    int steps = rollout->getPosesConst().cols();

    Eigen::MatrixXs posError = rollout->getPosesConst() - sensorPositions;

    // std::cout << "Pos In Buffer: " << std::endl << sensorPositions <<
    // std::endl; std::cout << "Pos In Traj  : " << std::endl <<
    // rollout->getPosesConst() << std::endl;
...
std::shared_ptr<LossFn> getSSIDVelPosLoss()
{
  TrajectoryLossFn loss = [](const TrajectoryRollout* rollout) {
    ...
    Eigen::MatrixXs sensorPositions
        = rawPos.block(0, 1, rawPos.rows(), rawPos.cols() - 1);
    Eigen::MatrixXs sensorVelocities
        = rawVel.block(0, 1, rawVel.rows(), rawVel.cols() - 1);
    int steps = rollout->getPosesConst().cols();
...
WorldPtr createWorld(s_t timestep)
{
  WorldPtr world = dart::utils::UniversalLoader::loadWorld(
      "dart://sample/skel/cartpole.skel");
...
Eigen::VectorXs rand_normal(
    size_t length, s_t mean, s_t stddev, std::mt19937 random_gen)
{
...
void recordObsWithNoise(
    size_t now,
    SSID* ssid,
    WorldPtr realtimeWorld,
    s_t noise_scale,
    std::mt19937 random_gen)
{
  ssid->registerLock();
  Eigen::VectorXs control_force = realtimeWorld->getControlForces();
  // Eigen::VectorXs force_eps = rand_normal(control_force.size(), 0,
  // noise_scale, random_gen);
  ...
  Eigen::VectorXs velocity_eps
      = rand_normal(velocity.size(), 0, noise_scale, random_gen);
...
Eigen::MatrixXs std2eigen(std::vector<Eigen::VectorXs> record)
{
  ...
  Eigen::MatrixXs eigen_record
      = Eigen::MatrixXs::Zero(num_record, record[0].size());
  std::cout << "Size: " << eigen_record.cols() << " " << eigen_record.rows()
            << std::endl;
...
TEST(REALTIME, CARTPOLE_MPC_MASS)
{
  ...
  ssid.setInitialPosEstimator(
      [](Eigen::MatrixXs sensors, long) { return sensors.col(0); });
  ...
  ssid.registerInferListener([&](long,
                                 ...
                                 long,
                                 bool steadyFound) {
    // mpcLocal.recordGroundTruthState(time, pos, vel, mass); //TODO: This will
    // cause problem ... But Why If there are more than one type of parameters
    // will this work?
    if (steadyFound)
    {
  ...
  ticker.registerTickListener([&](long now) {
    if (renderIsReady)
    {
      ...
      Eigen::VectorXs force_eps
          = rand_normal(mpcforces.size(), 0, noise_scale, rand_gen);
    ...
    else if (server.getKeysDown().count("o") || cnt == 600)
    {
      ...
      params(2) = 0.1;
      realtimeUnderlyingWorld->setLinkMassIndex(params(0), ssid_index);
      realtimeUnderlyingWorld->setJointDampingCoeffIndex(
          params.segment(2, 1), 1);
    }
    ...
    else if (server.getKeysDown().count("p") || cnt == 940)
    {
      if (record)
      {
        ...
        ssid.saveCSVMatrix(
            "/workspaces/nimblephysics/dart/realtime/saved_data/timeplots/"
            "cartpole_10_real_"
                + std::to_string(filecnt) + ".csv",
            real_params);
        ssid.saveCSVMatrix(
            "/workspaces/nimblephysics/dart/realtime/saved_data/timeplots/"
            "cartpole_10_identified_"
                + std::to_string(filecnt) + ".csv",
            sysid_params);
        filecnt++;
      }
    ...
    if (renderIsReady)
    {
      ...
      // }
      recordObsWithNoise(
          now, &ssid, realtimeUnderlyingWorld, noise_scale, rand_gen);
    ...
    if (total_steps % 5 == 0)
    {
      ...
      server.createText(
          key,
          "Cur Params: " + std::to_string(id_params(0)) + " "
              + std::to_string(id_params(1)) + " "
              + std::to_string(id_params(2))
              + "Real Params: " + std::to_string(params(0)) + " "
