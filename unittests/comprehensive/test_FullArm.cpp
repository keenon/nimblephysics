...
TEST(REALTIME, CARTPOLE_MPC_MASS)
{
  ...
  std::vector<size_t> ssid_idx{6,7};
  ...
  ssid.setSSIDIndex(Eigen::Vector2i(6, 7));  
  ...
  // Create Goal
  Eigen::VectorXs runningStateWeight = Eigen::VectorXs::Zero(2 * 7);
  Eigen::VectorXs runningActionWeight = Eigen::VectorXs::Ones(7) * 0.01;
  Eigen::VectorXs finalStateWeight = Eigen::VectorXs::Ones(2 * 7) * 20.0;

  runningStateWeight.segment(0, 7) = Eigen::VectorXs::Ones(7) * 0.01;
  finalStateWeight.segment(7, 7) *= 0.5;
  ...
  */
  ...
  masses(0) = world->getLinkMassIndex(5);
  masses(1) = world->getLinkMassIndex(6);
